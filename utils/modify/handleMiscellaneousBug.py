...
def handle_miscellaneous_bugs(AddressDataForOutput) -> None:
    """
    args: AddressDataForOutput (すでに分割された住所データ, 出力用には整形済み)
    return: void
    ...
    for index in range(len(AddressDataForOutput.address4)):
        if re.search("[0-9]+(ー[0-9]+)*", AddressDataForOutput.address4[index]):
        ...
        if AddressDataForOutput.address4[index] != "" and AddressDataForOutput.address3[index] == "":
            ...
            if re.search(
                ...
                AddressDataForOutput.address2[index] = (
                    AddressDataForOutput.address2[index] + AddressDataForOutput.address4[index]
                )
                AddressDataForOutput.address4[index] = ""
