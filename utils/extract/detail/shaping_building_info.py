...
def shaping_and_extracting_building_info(AddressDataForFormatting, manipulated_others_tail: list) -> None:
    ...
    def cutting_number_from_last2(index: int):
        ...
        if re.search("-([0-9]+)$", AddressDataForFormatting.house_number[index]) is None:
            ...
            print(re.search("-([0-9]+)$", AddressDataForFormatting.house_number[index]))
            print(AddressDataForFormatting.house_number[index])
            print("something wrong1")  # for debug
        else:
            start: int = re.search("-([0-9]+)$", AddressDataForFormatting.house_number[index]).start()
            end: int = re.search("-([0-9]+)$", AddressDataForFormatting.house_number[index]).end()
            if end != len(AddressDataForFormatting.house_number[index]):
                print("somethin wrong2")  # for debug
            # start+1 にしているのは-{数字}階となっているので - を除いている
            AddressDataForFormatting.building_detail_info[index] = AddressDataForFormatting.house_number[index][start + 1 : end] + "階"
    ...
    def cutting_number_from_last3(index: int):
        ...
        if re.search("-([0-9]+)$", AddressDataForFormatting.house_number[index]) is None:
            ...
            if end != len(AddressDataForFormatting.house_number[index]):
                print("somethin wrong2")  # for debug
            # start+1 にしているのは-{数字}階となっているので - を除いている
            AddressDataForFormatting.building_detail_info[index] = AddressDataForFormatting.house_number[index][start + 1 : end] + "号"
            AddressDataForFormatting.house_number[index] = AddressDataForFormatting.house_number[index][:start]
